    ...
    return (
      <div className="">
        ...
        {showForm && (
          <div>
            {" "}
            <span onClick={() => setShowForm(false)}> X </span> <Form />{" "}
          </div>
        )}
        <FlashcardArray cards={dSet.cards} setId={dSet.name} />
        <Button onClick={() => setShowForm(true)}> Add a flashcard</Button>
        <Button onClick={() => navigate(`/tests/${dSet.name}`)}>
          {" "}
          Test Flashcard
        </Button>
        <Button onClick={() => deleteSet(dSet.id)} className="bg-red-800">
          {" "}
          Delete Set
        </Button>
      </div>
